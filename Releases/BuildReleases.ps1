...
$platforms = @("linux-x64", "osx-x64", "win-x64" )
...
foreach ($platform in $platforms) {
    ...
    if (Test-Path $archivePath) {
        ...
        if (-not (Test-Path $projectPath)) {
            ...
            dotnet publish "../Source/PathOfEmulator.API/PathOfEmulator.API.csproj" -p:PublishProfile="$publishProfile" --output "$projectPath" -p:DebugType=None
        } else {
